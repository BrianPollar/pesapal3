...
class Pesapal {
   ...
   */
    async submitOrder(paymentDetails, productId, description) {
        exports.logger.info('details are', paymentDetails);
        const gotToken = await this.relegateTokenStatus().catch(err => err);
        if (gotToken instanceof Error) {
            return new Promise((resolve, reject) => reject(gotToken));
        }
        ...
        exports.logger.debug('IPNS ARE', this.ipns);
        ...
        return new Promise((resolve, reject) => {
            axios_1.default
                ...
                const response = res.data;
                if (response.error) {
                    exports.logger.error('PesaPalController, submitOrder error', response.error);
                    if (typeof response.error === 'string') {
                        reject(new Error(response.error));
                    }
                    else {
